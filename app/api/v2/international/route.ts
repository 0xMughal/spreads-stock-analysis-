/**
 * Your Own Stock API - International
 *
 * GET /api/v2/international
 *
 * Serves International stocks data from YOUR database.
 */
...
export async function GET(request: NextRequest) {
    ...
    return NextResponse.json(
      {
        ...
        fallback: 'Use /api/international for external API fallback',
      },
      { status: 503 }
    )
  }
  ...
  try {
    const quotes = await getDatasetQuotes('international')
    ...
    console.error('Error fetching International from database:', error)
    return NextResponse.json(
