...
/**
 * GET /api/v1/sp500
 * Bulk endpoint to fetch all S&P 500 stocks
 *
 * Query params:
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cacheKey = `stocks:sp500:${user!.tier}`
    ...
    if (cached && cached.stocks && cached.stocks.length > 0) {
      ...
      console.log(`[API v1] Cache hit for tier ${user!.tier} - ${stocks.length} stocks, age: ${cacheAge}s`)
    } else {
      // Cache miss - try to get from the free tier cache (always most fresh)
      const freeCache = await kv.get<CachedData>('stocks:sp500')
      ...
      if (freeCache && freeCache.stocks && freeCache.stocks.length > 0) {
        ...
        console.log(`[API v1] Using fallback cache - ${stocks.length} stocks`)
        ...
        return NextResponse.json(
          {
            ...
            message: 'Stock data is being fetched. Please try again in a few moments.',
          },
          { status: 503 }
        )
      }
